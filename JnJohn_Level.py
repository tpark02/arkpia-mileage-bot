...
async def on_message(message):
    ...
    if res is None:
        userinfo.insert_one({ "_id" : str(message.author.id), "author" : str(message.author), "level" : 0, "exp" : 0, "mileage" : 0, "msg" : 0, "todayMsg" : 0, "todayExp" : 0 })
        print(f"New Member Insert id : { message.author.id }, author : { message.author }")
    else:
        exp = res["exp"]
        ...
        if todayExp + 15 <= 200:
            ...
            if level_start < level_end:
                ...
                print(f" author : {str(message.author)}, level_start : {level_start}, leve_end : {level_end}, exp : {exp}, mileage : {mileage}, todayExp : {todayExp}, todayMsg : {todayMsgCnt}")
        else:
            userinfo.update_one({"_id": str(message.author.id) }, {"$set": {"todayMsg": todayMsgCnt }})
